...
from albert_dataloader import make_albert_dataloaders,albert_get_params_for_weight_decay_optimization
# model_params={
#     "MODEL":"albert-xxlarge-v2",             # model_type: albert-xxlarge-v2
# }
def get_model(args):
    """Build the model."""
    print_rank_0('building Albert model ...')
    config = AlbertConfig(num_hidden_layers=args.num_layers)
    #model = AlbertModel(config).from_pretrained("albert-xxlarge-v2")
...
def get_optimizer(model, args):
    ...
    while isinstance(model, (DDP, FP16_Module)):
        model = model.module
    param_groups = albert_get_params_for_weight_decay_optimization(model)
...
def forward_step(data_iterator, model, args, timers,tokenizer):
    ...
    # Forward model.
    # output = model(input_ids = ids, attention_mask = mask, labels=lm_labels)
...
def get_train_val_test_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        (train_data, val_data, test_data), num_tokens, \
                eod_token = make_albert_dataloaders(args)
