...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    checkpoint_folder = f"output1/{PREFIX_CHECKPOINT_DIR}-{self.state.global_step+1}"
                    filename=checkpoint_folder+"/gradients.bin"
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        print(output_dir)
        # save model into bin
        # f_param = open(output_dir+"/pytorch_model.bin_c","wb")
        # f_param.write(struct.pack("<i", self.model.num_parameters()))
        # for param in model.named_parameters():
        #     data_param = torch.flatten(param[1])
        #     for v in data_param.tolist():
        #         f_param.write( struct.pack("<f", v))
        
        # if we should start the computation
        # we start the computation every 50 steps
        # os.system("python dataprocess.py 0 48 ")
        if self.state.global_step % 40 ==0 and self.state.global_step != 0:
            # set_start_method('spawn')
            # change the data formart of each model
            if self.state.global_step-40==0 :
                start = str(2)
            else:
                start = str(self.state.global_step - 40 )
            os.system("python torch2bin.py "+ start+ " "+str(self.state.global_step) + " pytorch_model.bin")
    
            os.system("python torch2bin.py "+ start+ " "+str(self.state.global_step) + " gradients.bin")
            # process parameters
            os.system("./ana "+ str(int(start)+1) + " "+str(self.state.global_step) + " 1" +" >> model.ckp")
            # process gradients
            os.system("./ana "+ str(int(start)+1) + " "+str(self.state.global_step) + " 0" + " >> gra.ckp")

            # delete every thing.
            for i in range(int(start), self.state.global_step):
                os.system("rm -rf " + "/home/cc/scripts/output1/checkpoint-" + str(i))
